...
document.getElementById('isbn').addEventListener('keydown', function(event) {
    if (event.key === 'Enter') {
        ...
        if (isbn.length === 13) {  // ISBN-13
            fetch(`https://www.googleapis.com/books/v1/volumes?q=isbn:${isbn}`)
                ...
                .then(data => {
                    if (data.items && data.items.length > 0) {
                        ...
                        const series = book.series ? book.series.join(', ') : '';  // Laisser vide si pas de série
                        const tome = '';  // Optionnel, si tu as un champ "tome"
                        ...
                        document.getElementById('tome').value = tome;
            ...
            alert("Veuillez entrer un ISBN valide de 13 caractères.");
        }
    }
});
</script>
