<?php if(isset($_GET['s']) AND !empty($_GET['s'])){
        $s = $_GET['s'];
            ...
            <div class="bordure">
                ...
                <?php if($_SESSION['grade'] != 0){?><form method="get" action="<?php if(!isset($gestion)){ ?>gestion/<?php } ?>add-emprunt.php"><input type="hidden" name="id" value="<?= htmlspecialchars($books['id']); ?>"/><input type="submit" value="Ajouter un emprunt"/></form><?php } ?>
            </div>
            <br />
