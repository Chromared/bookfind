<?php if(isset($_GET['s']) AND !empty($_GET['s'])){
        $s = $_GET['s'];
            ...
            <div class="bordure">
                ...
                <?php if($_SESSION['grade'] != 0){ if($books['statut'] == 0){?><form method="get" action="<?php if(!isset($gestion)){ ?>gestion/<?php } ?>add-emprunt.php"><input type="hidden" name="id" value="<?= htmlspecialchars($books['id']); ?>"/><button type="submit" value="validate">Emprunter ce livre</button></form>
                <?php }elseif($books['statut'] == 1){ ?><form method="post" action="<?php if(!isset($gestion)){ ?>gestion/<?php } ?>#"><input type="hidden" name="id" value="<?= htmlspecialchars($books['id']); ?>"/><button type="submit" value="validate">Retourner cet emprunt</button></form><?php }} ?>
            </div>
            <br />
